...
```{r}
data_classed <- rbind(junk, Typical_Chatter, meaningful,ccdata)
...
```{r}
set.seed(1234)
wordcloud(words = data_classed_token$word,
...
token_by_class <- data_classed_token %>%
  count(type, class,author, word, sort = TRUE) %>%
  group_by(class) %>%
  #top_n(15) %>%
  ungroup()
```
...
```{r}
data_classed$length <- sapply(data_classed$message, function(x) length(unlist(strsplit(as.character(x), "\\W+"))))
...
data_classed_token %>%
  ...
  ungroup() %>%
  filter(n >= 100) %>%
  mutate(n = ifelse(class == "meaningful", -n, n)) %>%
  mutate(word = reorder(word, n)) %>%
  ggplot(aes(n, word, fill = class)) +
  geom_col() +
  labs(x = "Contribution to record classification", y = NULL)
...
```{r}
#data_lda <- LDA(data$message, k = 2, control = list(seed = 1234))
#data_lda
```
...
```{r}
#fire_term_counts <- data_classed %>%
#  extract(message, "fire", "(\\d+)", convert = TRUE) %>%
