...
'igraph','ggraph','DT','topicmodels','xts','dygraphs','sf',
'tmap','rgdal','raster','sp','patchwork','hrbrthemes')
for (p in packages){
...
**6.Visualizing words contribution to record classification by bar chart**

we examine how often associated words are preceded to one specific record type. We would like to explore which words contributed the most in different records types. The below code chunk was used to visualize the word contribution between "junk" & "meaningful and "junk" and "typical_chatter"record types with bar plot.
...
```
From the results shown in 8 topic model, we can 
...
Using the below code chunk, we could plot the risk level of Fire in Dancing Dolphin.
...
Using the below code chunk, we could plot the risk level of Hit & Run Accident. 
...
Using the below code chunk, we could plot the risk level of shooting and standoff event. 
