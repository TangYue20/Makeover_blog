...
```{r}
#Typical_Chatter <- data %>% filter_at(.vars = vars(message, author), 
#(str_detect(., pattern ="#POKlove#Lucio|#getoverit
#  |#baa-baa|#HomelessAwareness|#trylove|#pictures|AbilaAllFaith
#  |#nobodycares|#wishfulthinking|Viktor-E|
#  |#hogwash|#RememberElian|#standoff|work from home|
#  #blackvansrules|#schaber|#abilacityPark|#downwithkronos|FriendsOfKronos
#  |#RememberElianjaquesjoyce101|michelleR|SaveOurWildlands|
#  AbilaAllFaith|footfingers|GreyCatCollectibles|luvwool")))
```

```{r}
meaningful<- mbdata %>% filter_at(.vars = vars(message, author), 
any_vars(str_detect(., pattern ="#gowaway|#POKrally|van|gun|fire|rally|shooting|#AlibaPost|#CentralBulletin|
  ...
  |#kidnapping|#troubleatgelato|#Abila|#burnabila|#hostage| #repent|#dansingdolfinfire|#jerkdrivers|#AFD|#APDSWAT|
...
In this code chunk below, unnest_tokens() function from <span style="color:blue">*tidytext*</span> package is used to split the sentences into tokens, while stop_words() is used to remove stop-words.
