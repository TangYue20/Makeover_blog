...
```{r}
#fire_term_counts <- mb_usenet_words%>%
#  count(date, word) %>%
#  extract(word, "fire", "(\\d+)", convert = TRUE) %>%
#  complete(fire, word, fill = list(count = 0)) %>%
#  group_by(fire) %>%
#  mutate(fire_total = sum(count))
...
```{r}
rally_count <- usenet_words %>% 
  filter(str_detect(word,"POKRally|rally|park|Rallypark|Parkcheck|POKliesinthepark|POKRallyinthepark")) %>%
  group_by(date) %>%
  count(date)
...
```{r}
#rally_count$date <- ymd_hms(rally_count$datet)
...
```{r}
fire_count <- usenet_words %>% 
  filter(str_detect(word,"fire|Dancing|Dolphin|Department")) %>%
  group_by(date) %>%
  count(date)

ggplot(data = fire_count, aes(x = date, y = n, color = n )) +
    geom_line()
```
