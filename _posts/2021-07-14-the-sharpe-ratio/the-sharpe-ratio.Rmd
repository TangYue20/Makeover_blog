...
```{r}
junk <- data %>%
  filter(str_detect(message,"Grammar|#artists|#badcreadit|#cars|followme|#nobanks"))
  #filter(str_detect(author,"KronosQuoth|junkman|carjunkers"))
           
```

```{r}
meaningful <- data %>%
  filter(str_detect(message,"#gowaway|#POKrally|#POK|#AlibaPost|#CentralBulletin|#POKRallyinthePark|#IntNews")) %>%
...
```{r}
spam <- data %>%
  filter(str_detect(message, "#POKlove#Lucio|#getoverit|#baa-baa")) %>%
  filter(str_detect(author,"AbilaPost|anaregents|Clevvah4Evah"))
```
