...
### 2.4 Text proprecessing

In this step, we would like to clean text, including punctuation removal, converting lowe case, stop word removal, extra white space removal and stemming.
...
```{r}
cleaned_junk <- junk %>%
  filter(str_detect(message, "^[^>]+[A-Za-z\\d]")
         | message == "",
         !str_detect(message, 
                     "writes(:|\\.\\.\\.)$"),
         !str_detect(message, 
