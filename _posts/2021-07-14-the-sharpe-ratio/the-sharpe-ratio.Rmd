...
shooting_count <- usenet_words %>% 
  filter(str_detect(word,"shooting|standoff|^gun|shooter|shooting fire|
                    ambulance|Gelatogalorestandofff|Gelato Galore")) %>%
  group_by(date) %>%
  count(word,date)
...
Based on record provided and key words, we can see four social events happened from 17:00 pm to 21:30. they are：
