...
In this step, we would like to use package <span style="color:blue">*stringr*</span> to clean special characters, including " @, < , ˜ " and none English words. Besides, there are a lot of repeat records which forward from other publisher that begin with "RT". To avoid the affect the word frequency, we move "RT" in ahead.
...
* classify Junk records: The  <span style="color:light blue">*str_detect()*</span> function is used to detect patterns of Junk records. The below code chunk is used to identify the junk reports.|
