...
gps <- data2 %>%
  filter(latitude >0) %>%
  filter(str_detect(message,"fire|Dolphin|black van|shotting|rally ")) %>%
...
```{r}
gps_location_selected <- gps_location %>%
  mutate(duplicate=duplicated(geometry)) %>%
...
tm_shape(bgmap) +
  tm_rgb(bgmap, r = 1,g = 2,b = 3,
       ...
       interpolate = TRUE,
       max.value = 255) +
  tm_shape(gps_location_selected) +
  tm_bubbles(size=10,alpha = 0.5, col="red") 
